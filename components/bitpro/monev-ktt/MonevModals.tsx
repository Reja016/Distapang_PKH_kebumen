...
interface MonevCameraModalProps {
...
export function MonevCameraModal({
  showCameraModal,
  videoRef,
  canvasRef,
  takePhoto,
  closeCamera,
}: MonevCameraModalProps) {
...
interface MonevPreviewPhotoModalProps {
  previewPhotoModal: { url: string; title: string } | null;
  onClose: () => void;
}

export function MonevPreviewPhotoModal({ previewPhotoModal, onClose }: MonevPreviewPhotoModalProps) {
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-slate-800/90 border-b border-slate-700 text-white">
          ...
          <div className="flex items-center gap-1 shrink-0 ml-2">
            <a
              ...
              download={
                previewPhotoModal.title.toLowerCase().includes('ttd') || previewPhotoModal.title.toLowerCase().includes('tanda tangan')
                  ? `${previewPhotoModal.title.replace(/[^a-zA-Z0-9_-]/g, '_')}.png`
                  : `${previewPhotoModal.title.replace(/[^a-zA-Z0-9_-]/g, '_')}.jpg`
              }
              className="h-8 px-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-xs"
