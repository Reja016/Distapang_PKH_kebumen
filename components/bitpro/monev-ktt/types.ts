...
// Generator Daftar Tahun Otomatis (Tahun Berjalan + 1 hingga 2019)
export const getInitialYears = () => {
...
export const DAFTAR_TAHUN = getInitialYears();
export const DAFTAR_JENIS_TERNAK = ['Sapi', 'Kambing', 'Domba', 'Ayam KUB', 'Ayam Petelur'];

export const buatNamaFileFoto = (namaKtt: string, id: string | number) => {
...
export function hitungKondisi(k: KondisiTernak) {
  ...
  const e = a - b - c + d;
  const f = (k.lahirJantan || 0) + (k.lahirBetina || 0) + (k.lahirBelumTahu || 0);
  const g = (k.matiAnakJantan || 0) + (k.matiAnakBetina || 0) + (k.matiAnakBelumTahu || 0);
  const h = (k.jualAnakJantan || 0) + (k.jualAnakBetina || 0) + (k.jualAnakBelumTahu || 0);
  const i = e + f - g - h;
  return { a, b, c, d, e, f, g, h, i };
}

export function migrasiKondisi(raw: any): KondisiTernak {
...
// ── UNGGAS DATA TYPES ──
export const DAFTAR_JENIS_UNGGAS = ['Ayam KUB', 'Ayam Petelur', 'Ayam Pedaging / Broiler', 'Itik / Bebek', 'Burung Puyuh'];

export const KONDISI_UNGGAS_KOSONG = {
  awalTotal: 0,
  kematian: 0,
  dijual: 0,
  rataanTelur: 0,
...
export function hitungKondisiUnggas(k: KondisiUnggas) {
  const populasiSaatIni = Math.max(0, (k.awalTotal || 0) - (k.kematian || 0) - (k.dijual || 0));
...
export interface SuratPernyataanData {
  ...
  desa: string;
  kecamatan: string;
  alamatKelompok: string;
  noTelp: string;
  isiPernyataan: string;
  tanggal: string;
  fotoTtdKetuaCap?: string | null;
}

export const SURAT_PERNYATAAN_KOSONG: SuratPernyataanData = {
  nama: '',
