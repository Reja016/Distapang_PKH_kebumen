    ...
    <footer className="border-t border-slate-200 dark:border-slate-800 bg-slate-900 text-slate-300 transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-10">
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl p-1.5 bg-white/10 border border-white/20 flex items-center justify-center shrink-0 backdrop-blur-xs">
              ...
              <div>
                <h3 className="text-lg font-black tracking-tight text-white flex items-center gap-1.5">
                  <span>SiMantap</span>
                  <span className="text-xs px-2 py-0.5 rounded-md font-bold bg-blue-500/20 text-blue-400 border border-blue-500/30">
                    PKH
                  </span>
                </h3>
                ...
                {FOOTER_CONFIG.sosmed.map((s, idx) => {
                  ...
                  if (isAvailable) {
                    return (
                      <a
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-700/80 bg-slate-800/80 hover:bg-slate-700 text-slate-200 text-xs font-semibold transition-all hover:border-slate-500 hover:text-white group shadow-xs"
                      >
                        {s.type === 'instagram' && (
                          <svg className="w-3.5 h-3.5 text-pink-400 group-hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="currentColor">
                        ...
                        {s.type === 'facebook' && (
                          <svg className="w-3.5 h-3.5 text-blue-400 group-hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/>
                          </svg>
                        )}
                        {s.type === 'youtube' && (
                          <svg className="w-3.5 h-3.5 text-red-400 group-hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-slate-800/80 bg-slate-900/50 text-slate-500 text-xs font-medium cursor-not-allowed select-none opacity-60 hover:opacity-75 transition-opacity"
                    >
                      {s.type === 'instagram' && (
                      ...
                      <span>{s.nama}</span>
                      <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400 bg-slate-800/80 px-1 py-0.2 rounded border border-slate-700/60">
                        Segera
                      </span>
                    </div>
                  );
          ...
          {/* KOLOM 2: LAYANAN UTAMA & NAVIGASI (Span 2 Kolom) */}
          <div className="lg:col-span-3 space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-white border-b border-slate-800 pb-2 flex items-center gap-1.5">
            ...
            <ul className="space-y-2 text-xs">
              {FOOTER_CONFIG.layananUtama.map((item, idx) => (
                <li key={idx}>
                  <a
                    href={item.href}
                    className="text-slate-400 hover:text-white transition-colors flex items-start gap-1.5 group"
                  >
                    <ChevronRight size={13} className="text-slate-600 group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all shrink-0 mt-0.5" />
                    <span>{item.nama}</span>
                  </a>
                </li>
              ))}
          ...
          <div className="lg:col-span-4 space-y-3">
            ...
            <div className="space-y-2.5 text-xs text-slate-400">
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="font-semibold text-slate-200">Kantor Dinas Pertanian &amp; Pangan</p>
              ...
              {Boolean(FOOTER_CONFIG.kontak.jamKerja && FOOTER_CONFIG.kontak.jamKerja.trim() !== '') && (
                <div className="flex items-start gap-2.5 pt-1">
                  ...
                  <div>
                    <p className="font-semibold text-slate-200">Jam Pelayanan Kantor:</p>
                    <p className="text-[11px] text-slate-400">{FOOTER_CONFIG.kontak.jamKerja}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-3 text-xs">
          <span className="text-[11px] font-bold text-slate-400">Tautan Lembaga Terkait:</span>
      ...
      <div className="border-t border-slate-800 bg-slate-950 py-4 px-4 sm:px-6 lg:px-8 text-xs text-slate-500">
